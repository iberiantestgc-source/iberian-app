...
export default function TestScreen() {
  ...
  /*
   * Si no hay test, redirigimos automáticamente a la configuración.
   * Esto evita el mensaje de error cuando se abre desde la pestaña Tests.
   */
  useEffect(() => {
    if (!test) {
      router.replace('/test/setup' as any);
    }
  }, [test]);
  ...
  /*
   * Mientras redirige (cuando no hay test), mostramos un loading.
   */
  if (!test) {
    return (
      <View
  ...
  /*
   * A partir de aquí TypeScript sabe que existe,
   * pero utilizamos una referencia estable para evitar
   * problemas de narrowing dentro de callbacks.
   */
  const currentTest: GeneratedTest = test;
